params = {
    "setup": {"seed": 42},  # Random seed for random and torch packages
    "environment": {
        "map": "./submodules/cmapd/AM/env/grid.map",  # Filepath of the map file
        "max_collective_size": 20,  # Maximum collective size (hard coded into environment.py) -> An agent can complete up to max_collective_size tasks in this scenario
        "capacity": 1,  # Maximum agent capacity (hard coded into environment.py) -> An agent can carry up to capacity tasks simultaneously
    },
    "model": {  # Embedding -> N x (Attention -> Feedforward) -> Decoder
        "input_size": [
            ...
            21,
            35,
        ],  # Input feature size (embedding layer pytorch) [pickup_x, pickup_y, delivery_x, delivery_y]
